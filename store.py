...
def connect():
    ...
    # Latere kolommen: CREATE IF NOT EXISTS voegt ze niet toe aan een bestaande tabel.
    kolommen = {r[1] for r in con.execute('PRAGMA table_info(trips)')}
    if 'zakelijk' not in kolommen:
        con.execute('ALTER TABLE trips ADD COLUMN zakelijk INTEGER NOT NULL DEFAULT 0')
        con.commit()
    return con
