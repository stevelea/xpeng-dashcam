...
def build(con=None):
    ...
    bewaard = {r['start_ts']: (r['km'], r['measured'], r['note'])
               for r in con.execute('SELECT start_ts, km, measured, note FROM trips')}
    con.execute('DELETE FROM trips')
    for g in groepen:
        start, eind = g[0], g[-1]
        seconden = int((eind - start).total_seconds()) + 60
        key = start.isoformat(timespec='seconds')
        km, measured, note = bewaard.get(key, (None, 0, None))
        con.execute(
            ...
            (start.strftime('%Y-%m-%d'), key, eind.isoformat(timespec='seconds'),
             len(g), seconden, km, measured, note))
    con.commit()
